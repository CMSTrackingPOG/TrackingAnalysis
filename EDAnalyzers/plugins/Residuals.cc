...
//	     std::cout << i << " " << trigName << " L1=" << L1prescale << " HLT=" << HLTprescale << std::endl;
   ...
   if( doTruth && !runOnData )
	...
	for( unsigned int ipv=0;ipv<pvr.size();ipv++ )
	  {	     	
	     ...
	     std::vector<int> pv_mc_isPrimaryVertex;
	     std::vector<int> pv_mc_isSecondaryVertex;
	     std::vector<int> pv_mc_isTertiaryVertex;
	     std::vector<int> pv_mc_isSignalEvent;
	     std::vector<int> pv_mc_isBWeakDecay;
	     std::vector<int> pv_mc_isCWeakDecay;
	     std::vector<int> pv_mc_isTauDecay;
	     std::vector<int> pv_mc_isKsDecay;
	     std::vector<int> pv_mc_isLambdaDecay;
	     std::vector<int> pv_mc_isJpsiDecay;
	     std::vector<int> pv_mc_isXiDecay;
	     std::vector<int> pv_mc_isOmegaDecay;
	     std::vector<int> pv_mc_isSigmaPlusDecay;
	     std::vector<int> pv_mc_isSigmaMinusDecay;
	     std::vector<int> pv_mc_isLongLivedDecay;
	     
	     std::vector<int> pv_mc_isKnownProcess;
	     std::vector<int> pv_mc_isUndefinedProcess;
	     std::vector<int> pv_mc_isUnknownProcess;
	     std::vector<int> pv_mc_isPrimaryProcess;
	     std::vector<int> pv_mc_isHadronicProcess;
	     std::vector<int> pv_mc_isDecayProcess;
	     std::vector<int> pv_mc_isComptonProcess;
	     std::vector<int> pv_mc_isAnnihilationProcess;
	     std::vector<int> pv_mc_isEIoniProcess;
	     std::vector<int> pv_mc_isHIoniProcess;
	     std::vector<int> pv_mc_isMuIoniProcess;
	     std::vector<int> pv_mc_isPhotonProcess;
	     std::vector<int> pv_mc_isMuPairProdProcess;
	     std::vector<int> pv_mc_isConversionsProcess;
	     std::vector<int> pv_mc_isEBremProcess;
	     std::vector<int> pv_mc_isSynchrotronRadiationProcess;
	     std::vector<int> pv_mc_isMuBremProcess;
	     std::vector<int> pv_mc_isMuNuclProcess;
	     std::vector<int> pv_mc_isUnknown;
	     
	     std::vector<int> pv_mc_inVolume;
   ...
   for( unsigned int ipv=0;ipv<pvr.size();ipv++ )
	...
	std::vector<int> pv_trk_isHighPurity( nTracks );
	...
	std::vector<int> pv_trk_hasPixelBarrelLayer1( nTracks );
	std::vector<int> pv_trk_hasPixelEndcapLayer1( nTracks );
	std::vector<int> pv_trk_hasPixelBarrelLayer2( nTracks );
	std::vector<int> pv_trk_hasPixelEndcapLayer2( nTracks );
	std::vector<int> pv_trk_hasPixelBarrelLayer3( nTracks );
	std::vector<int> pv_trk_hasPixelEndcapLayer3( nTracks );
	std::vector<int> pv_trk_hasPixelBarrelLayer4( nTracks );
	std::vector<int> pv_trk_hasPixelEndcapLayer4( nTracks );
   ...
   for( TrackCollection::const_iterator itk = tracks->begin(); itk != tracks->end(); ++itk )
	...
	if( doTruth && !runOnData )
	  {	     
	     ...
	     std::vector<int> trk_mc_isFake;
	     std::vector<int> trk_mc_isBad;
	     std::vector<int> trk_mc_isBadInnerHits;
	     std::vector<int> trk_mc_isSharedInnerHits;
	     std::vector<int> trk_mc_isSignalEvent;
	     std::vector<int> trk_mc_isTrackerSimHits;
	     std::vector<int> trk_mc_isBottom;
	     std::vector<int> trk_mc_isCharm;
	     std::vector<int> trk_mc_isLight;
	     std::vector<int> trk_mc_isMuon;
		       
	     std::vector<int> trk_mc_isBWeakDecay;
	     std::vector<int> trk_mc_isCWeakDecay;
	     std::vector<int> trk_mc_isChargePionDecay;
	     std::vector<int> trk_mc_isChargeKaonDecay;
	     std::vector<int> trk_mc_isTauDecay;
	     std::vector<int> trk_mc_isKsDecay;
	     std::vector<int> trk_mc_isLambdaDecay;
	     std::vector<int> trk_mc_isJpsiDecay;
	     std::vector<int> trk_mc_isXiDecay;
	     std::vector<int> trk_mc_isOmegaDecay;
	     std::vector<int> trk_mc_isSigmaPlusDecay;
	     std::vector<int> trk_mc_isSigmaMinusDecay;
	     std::vector<int> trk_mc_isLongLivedDecay;
	     
	     std::vector<int> trk_mc_isKnownProcess;
	     std::vector<int> trk_mc_isUndefinedProcess;
	     std::vector<int> trk_mc_isUnknownProcess;
	     std::vector<int> trk_mc_isPrimaryProcess;
	     std::vector<int> trk_mc_isHadronicProcess;
	     std::vector<int> trk_mc_isDecayProcess;
	     std::vector<int> trk_mc_isComptonProcess;
	     std::vector<int> trk_mc_isAnnihilationProcess;
	     std::vector<int> trk_mc_isEIoniProcess;
	     std::vector<int> trk_mc_isHIoniProcess;
	     std::vector<int> trk_mc_isMuIoniProcess;
	     std::vector<int> trk_mc_isPhotonProcess;
	     std::vector<int> trk_mc_isMuPairProdProcess;
	     std::vector<int> trk_mc_isConversionsProcess;
	     std::vector<int> trk_mc_isEBremProcess;
	     std::vector<int> trk_mc_isSynchrotronRadiationProcess;
	     std::vector<int> trk_mc_isMuBremProcess;
	     std::vector<int> trk_mc_isMuNuclProcess;
	     
	     std::vector<int> trk_mc_isFromBWeakDecayMuon;
	     std::vector<int> trk_mc_isFromCWeakDecayMuon;
	     std::vector<int> trk_mc_isDecayOnFlightMuon;
	     std::vector<int> trk_mc_isFromChargePionMuon;
	     std::vector<int> trk_mc_isFromChargeKaonMuon;
	     
	     std::vector<int> trk_mc_isPrimaryVertex;
	     std::vector<int> trk_mc_isSecondaryVertex;
	     std::vector<int> trk_mc_isTertiaryVertex;
	     
	     std::vector<int> trk_mc_isUnknown;
